...
public partial class BetaWindow : Window
{
    ...
    public BetaWindow(AppSettings settings, LoggerService logger, WarframeUpdateManager updateManager)
    {
        InitializeComponent();
        ...
        txtGameFolder.Text = _settings.WarframeInstallFolder;
        ...
        _logger.LogInfo("[BETA] Warframe AIO Tool — Updater Beta initialization complete.");
        _logger.LogInfo($"[BETA] Target folder: {_settings.WarframeInstallFolder}");
    }
    ...
    private void btnBrowseFolder_Click(object sender, RoutedEventArgs e)
    {
        var dialog = new Microsoft.Win32.OpenFolderDialog
        {
            ...
            InitialDirectory = txtGameFolder.Text
        };

        if (dialog.ShowDialog() == true)
        {
            txtGameFolder.Text = dialog.FolderName;
            _settings.WarframeInstallFolder = dialog.FolderName;
            _settings.Save();
            _logger.LogInfo($"[SETTINGS] Install path updated: {dialog.FolderName}");
    ...
    private async void btnTestSingleFile_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (_updateManager.CurrentManifest == null)
        {
    ...
    private void OnProgressUpdated(object? sender, DownloadProgressEventArgs e)
    {
        Dispatcher.Invoke(() =>
        {
            ...
            if (e.TotalBytes > 0)
            {
                ...
                lblProgressDetails.Text = $"{WarframeManifestEntry.FormatBytes(e.BytesReceived)} / {WarframeManifestEntry.FormatBytes(e.TotalBytes)} ({speedText})";
            }
            else
            {
                lblProgressDetails.Text = $"{WarframeManifestEntry.FormatBytes(e.BytesReceived)} downloaded";
            }
        });
    }
